...
| Qdrant | 오픈소스, 운영급, Rust 기반 빠름 | 학습 곡선 (Rust 컬렉션 / 페이로드 인덱싱) | **T-6 카드로 마이그레이션 완료 (2026-05-05, ADR 0009)** — 어댑터 패턴 + env 토글 |
...
→ **T-6 PR(2026-05-05)에서 어댑터 패턴 + 환경변수 토글로 마이그레이션 완료** (ADR 0009 참조).
...
### 면접 답변 한 줄
> *"프로토타입 단계라 ChromaDB로 빠르게 시작했고, T-6 카드에서 Qdrant로 마이그레이션 했습니다. 어댑터 패턴(`vector_store.py`)으로 추상 인터페이스 박고 환경변수 `VECTOR_STORE=chromadb|qdrant`로 토글하게 해서 호출자(`rag.py`) 0 변경 + 운영 사고 시 즉시 ChromaDB 복원 가능합니다. ChromaDB는 in-memory 모드라 영속성/멀티 인스턴스 동기화 약해서 운영 진입 시 Qdrant로 토글하는 구조에요. 청킹은 의미 단위 평균 500토큰 + 50토큰 오버랩, 임베딩은 Gemini 768차원 사용했습니다."*
