...
## 🔍 plan 검수 13 영역 가이드 (시니어 검수 본질 — 있는 것 + 없는 것 + 외부 영향 + 메타)

> 사용자 검수 / Claude Code 셀프 검수 모두 **13 영역 일관 형식**으로 적용. 영역 1-10은 plan 본문에 *"있는 것"* 검증, 영역 11-13은 *"plan에 없는 본질 점검"*.

### 영역 1 — 측정 0~5 (사전 측정)

- 각 측정 결과 plan 본문 박힘 (✅/⚠️/❌)
- 본질 조건(의존성/사용처/차원/Docker/회귀) 모두 커버
- 측정 결과 미포함 시 Step 0에서 실측 후 분기 명시

### 영역 2 — 5 가드 (G1-G5) + 메모리 위반 통합 점검

- **G1 본질 트리거 조건** (시간 cap X, 본질 조건 O — 의존성 충돌 / 광범위 사용처 / 시그니처 호환 X / 차원 불일치)
- **G2 Reversibility** + 롤백 1줄
- **G3 Done Definition** 80/100
- **G4 Round Cap** (재검토 X)
- **G5 First Principle** 본질 1줄
- **메모리 통합 위반 점검**:
  - #19 (PR 머지 패턴)
  - #21 (Big Bet 시 6 패턴)
  - #22 (5 가드)
  - #23 (Type 2 자동 진행)
  - #24 (Claude Code 사용 패턴)
  - #25 (컨디션 묻지 X)
  - #26 (답변 단순화)
  - #27 (반복 X)
  - #28 (시간 cap / 시간 분해 X)

### 영역 3 — WORK_PATTERNS 적용
...
- 신규 N건 (라인 추정)
- 수정 N건 (변경 위치)
- 미수정 보존 N건 (git diff 빈 출력 검증 의무)
- 라인 번호 가정 시 grep 자동 위치 확인 가능

### 영역 5 — 어댑터 / 핵심 설계

- 추상 인터페이스
- 호출자 인터페이스 0 변경 보장
- 자기 일관성 패턴 (Lazy Init / 환경변수 토글 / 응답 어댑터)

### 영역 6 — Step 분해

- 흐름 추적용 (시간 분해 X — 메모리 #28)
- G1 fallback 단계별 명시
...
- 신규 통합 테스트
- 보존 검증 (git diff 빈 출력)
- PR 게이트 (black / ruff / mypy / pytest cov 81%+)

### 영역 9 — 문서 갱신

- ADR 신규 7 항목 (상태 / 컨텍스트 / 결정 / 근거 / 트레이드오프 / 롤백 / 후속)
- TECH_DECISIONS / AGENTS 갱신 위치
- 면접 답변 보강

### 영역 10 — 자기 점검 체크리스트

- PRINCIPLES / SCENARIO / WORK_PATTERNS / TECH_DECISIONS 참조 명시
- pre-existing 보존
- 단일 squash commit
- Co-Authored-By 트레일러 X

### 영역 11 (신규) — 누락 위험 (plan에 없는 본질 점검)

- 보안 영향 (env 토글로 인한 인증 우회 / 비밀 노출)
- 성능 회귀 (어댑터 latency 증가)
- 멀티 인스턴스 동기화 (전환 시 데이터 일관성)
- 마이그레이션 중 다운타임 (서비스 중단)
- 백업 / 복구 시나리오 (데이터 손실 대응)

### 영역 12 (신규) — 외부 의존성 영향

- 다른 서비스 영향 (portfolio-service / auth-service / frontend)
- 배포 / CI 영향 (GitHub Actions / Jenkinsfile)
- 운영 모니터링 영향 (Prometheus / Grafana 메트릭)
- 백업 시스템 영향

### 영역 13 (신규) — 메타 검수 (시니어 시각)

- 다른 개발자가 plan 보고 같은 결과 만들 수 있나
- 6개월 후 본인이 봤을 때 *"왜 이렇게?"* 답 가능한가
- 면접관이 plan 보고 *"시니어다"* 판단할 시그널 있나
- plan 자체가 *"결정 근거 추적"* 시스템인가 (PRINCIPLES.md 원칙 5)
...
### T-6 plan 검수 사례 (실제 적용)

- **영역 1-10**: 본질 문제 0건 / 메모리 #28 위반 1건 (plan에 시간 분해 박혔음 — 미세) / 미세 점검 2건
- **영역 11 누락 위험**: 마이그레이션 중 다운타임 미명시 (시나리오 A 맥락이라 무시 가능)
- **영역 12 외부 영향**: CI 파이프라인 영향 미검수 (Qdrant Docker 추가 — Jenkinsfile 영향 후속 카드)
- **영역 13 메타 시그널**: TECH §1 인용 + 어댑터 패턴 + 환경변수 토글 = 시니어 시그널 강함

### 적용 주체

| 주체 | 역할 |
|---|---|
| **Claude (사용자 검수)** | 13 영역 일관 형식 적용. plan 받으면 영역별 표 박아 검수 결과 제출 |
| **Claude Code (plan 작성 시 셀프 검수)** | 영역 11-13 명시 의무. plan 본문에 "11. 누락 위험 / 12. 외부 영향 / 13. 메타 검수" 박음 |
...
| 2026-05-05 (v4) | **카테고리 F에 "검증 + 분기 + 머지 자동화" 패턴 추가** (메모리 #19 진화형) — 사용자 한 줄 프롬프트로 pre-existing 검증 + 자동 분기 + 머지 명령까지 묶는 흐름 박음. 실제 사례: T-2 PR #12 머지 직전 `git diff llm-service/app/config.py` 검증 분기. | T-2 머지 흐름에서 발견된 자기 일관성 패턴 정착 |
| 2026-05-05 (v5 / 본 문서) | **자기 일관성 패턴 다음에 "plan 검수 13 영역 가이드" 신규 섹션 추가** — 영역 1-10(있는 것 검증) + 영역 11 누락 위험(보안/성능/동기화/다운타임/백업) + 영역 12 외부 영향(서비스/CI/모니터링) + 영역 13 메타 시그널(다른 개발자/6개월 후 본인/면접관/결정 근거 추적). T-6 plan 실제 검수 사례 박음. Claude 사용자 검수 + Claude Code 셀프 검수 양쪽 적용. | 시니어 검수 본질 = "있는 것 + 없는 것 + 외부 영향 + 메타" 모두 커버. plan이 "결정 근거 추적" 시스템 자체가 되도록 강제 |
